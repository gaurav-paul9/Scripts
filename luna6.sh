...
# --- 5.1.1 Apply OPlus Camera Patches ---
echo "📸 Cloning and applying OPlus camera patches..."
git clone https://gaurav-paul9:${GH_TOKEN}@github.com/gaurav-paul9/lunaris-patches.git -b master .luna_patches --depth=1

PATCH_SUCCESS=true
if [ -f ".luna_patches/apply_patches.sh" ]; then
    bash .luna_patches/apply_patches.sh . || true
    
    # Check results for failures
    LATEST_LOG=$(ls -t .luna_patches/logs/results_*.txt 2>/dev/null | head -1)
    if [ -f "$LATEST_LOG" ]; then
        APPLIED_COUNT=$(grep -c "^OK" "$LATEST_LOG" || echo 0)
        SKIPPED_COUNT=$(grep -c "^SKIP" "$LATEST_LOG" || echo 0)
        FAILED_COUNT=$(grep -cE "^(FAIL|PARTIAL)" "$LATEST_LOG" || echo 0)
        
        if [ "$FAILED_COUNT" -gt 0 ]; then
            PATCH_SUCCESS=false
            # Extract failed commit subjects
            FAILED_LIST=$(grep -E "^(FAIL|PARTIAL)" "$LATEST_LOG" | awk -F'|' '{print $3}' | sed 's/^ *//' | head -10)
            send_telegram_message "⚠️ *OPlus Patches: Partial Success*
...
Build will continue..."
            ...
            # Show applied commit subjects
            APPLIED_LIST=$(grep "^OK" "$LATEST_LOG" | awk -F'|' '{print $3}' | sed 's/^ *//' | head -10)
            send_telegram_message "✅ *OPlus Patches Applied*
...
\`\`\`"
    ...
    send_telegram_message "⚠️ *Warning:* OPlus camera patches not found. Skipping."
fi
