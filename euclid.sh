...
# =======================
echo "Initializing Euclid repository..."
...
git clone https://github.com/LineageOS/android_hardware_oplus.git -b lineage-23.1 hardware/oplus --depth=1
...
git clone https://github.com/OnePlus12R-development/android_device_oneplus_aston-kernel.git -b sixteen-qpr1 device/oneplus/aston-kernel --depth=1
...
. build/envsetup.sh
lunch euclid_aston-bp3a-userdebug
...
ZIP_FILE=$(find "$OUTPUT_DIR" -type f -iname "Euclid*.zip" -printf "%T@ %p\n" | sort -n | tail -n1 | cut -d' ' -f2-)
...
if [[ -f "$ZIP_FILE" ]]; then
  ...
  if [[ "$FILE_ID" != "null" && -n "$FILE_ID" ]]; then
    ...
    UPLOAD_MESSAGE="🎉 *Euclid (Root) Upload Complete!*
