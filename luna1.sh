...
upload_to_pixeldrain() {
    local file_path=$1
    if [ -f "$file_path" ]; then
        # -u ":KEY" is the correct way to authenticate with Pixeldrain API
...
if [[ -f "$ZIP_FILE" ]]; then
    ...
    echo "Uploading Boot..."
    BOOT_URL=$(upload_to_pixeldrain "$BOOT_IMG")

    # Constructing Telegram Message (No literal \n, using actual line breaks)
...
🔹 [ROM ZIP]($ZIP_URL)"
    ...
    if [[ "$BOOT_URL" == "error" || "$BOOT_URL" == "missing" ]]; then
        FINAL_MESSAGE="$FINAL_MESSAGE
🔹 Boot Image: Upload Failed or Not Found ❌"
    else
        FINAL_MESSAGE="$FINAL_MESSAGE
🔹 [Boot Image]($BOOT_URL)"
    fi
