...
Exit code: \`$code\`  
See the attached log snippet below."
...
# Remove existing directories to prevent clone conflicts
...
if [ -f "$VIDEO_FILE" ]; then
    ...
    echo "⚠️ Warning: $VIDEO_FILE not found! Skipping patch."
...
live_monitor() {
    ...
    while true; do
        sleep 60
        if [ -f "$BUILD_LOG" ]; then
            # Extract progress percentage or task count
...
# Finding the zip and boot image
...
if [[ -f "$ZIP_FILE" ]]; then
    ...
    FINAL_MESSAGE+="⏱️ *Time:* \`$DURATION_FORMATTED\`\n"
    FINAL_MESSAGE+="💾 *Size:* \`$SIZE_HUMAN\`\n"
    FINAL_MESSAGE+="📦 *File:* \`$FILE_NAME\`\n"
    FINAL_MESSAGE+="🛡️ *MD5:* \`$MD5SUM\`\n"
    FINAL_MESSAGE+="📼 *Disk Left:* \`$END_DISK\`\n\n"
    FINAL_MESSAGE+="📱 *Downloads:*\n"

    ZIP_URL=$(upload_to_pixeldrain "$ZIP_FILE" "ROM")
    if [[ "$ZIP_URL" == "error" || "$ZIP_URL" == "missing" ]]; then
        FINAL_MESSAGE+="🔹 ROM ZIP: Upload Failed ❌\n"
    else
        FINAL_MESSAGE+="🔹 [ROM ZIP]($ZIP_URL)\n"
    fi

    BOOT_URL=$(upload_to_pixeldrain "$BOOT_IMG" "BOOT")
    if [[ "$BOOT_URL" == "error" || "$BOOT_URL" == "missing" ]]; then
        FINAL_MESSAGE+="🔹 Boot Image: Upload Failed or Not Found ❌\n"
    else
        FINAL_MESSAGE+="🔹 [Boot Image]($BOOT_URL)\n"
    fi
...
EOF
        ...
        if [[ "$JSON_URL" == "error" || "$JSON_URL" == "missing" ]]; then
            FINAL_MESSAGE+="🔹 OTA JSON: Upload Failed ❌\n"
        else
            FINAL_MESSAGE+="🔹 [OTA JSON]($JSON_URL)\n"
        fi
    fi
...
The build process completed, but the expected Lunaris ZIP was not found in the output folder."
fi
