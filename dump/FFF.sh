...
# --- Self-cache for streamed execution (Termux-safe & Linux-safe) ---
# If the script is being piped into bash, buffer it to a temp file and re-exec.
if [ -t 0 ]; then
  : # running from file, continue
else
  TMP="${PREFIX:-$HOME}/tmp/axion_auto_$$.sh"
  mkdir -p "$(dirname "$TMP")"
  cat > "$TMP"
  chmod +x "$TMP"
  exec "$TMP"
...
# --- Load secrets from .env (portable, safe when BASH_SOURCE missing) ---
...
if [ -f "$SCRIPT_DIR/.env/secrets.env" ]; then
  ...
  echo "❌ .env/secrets.env not found! Create .env/secrets.env next to the script or in current directory. Exiting."
  exit 1
fi
...
monitor_progress() {
  ...
  while kill -0 "$pid" 2>/dev/null; do
    ...
    ((p > 99)) && p=99
    local filled=$((p/5))
    local bar="$(printf '█%.0s' $(seq 1 $filled) 2>/dev/null; printf '░%.0s' $(seq $((filled+1)) 20) 2>/dev/null)"
...
rm -rf \
  ...
  out/target/product/aston/system \
  out/target/product/aston/product || true
send_raw "✅ *Cleanup Complete!*  
_Removed old sources and conflicts successfully._ 🧼"

# =========================================================
# 🧼 CLEAN BUILD
# =========================================================
send_raw "🧼 *Starting Clean Build...* 🚀"
repo init -u https://github.com/AxionAOSP/android.git -b lineage-23.0 --git-lfs || { send_raw "❌ repo init failed"; exit 1; }
rm -rf prebuilts/clang/host/linux-x86

send_raw "🔁 *Syncing sources via /opt/crave/resync.sh...* ⏳"
bash /opt/crave/resync.sh || { send_raw "❌ resync.sh failed"; exit 1; }

# =========================================================
# 🔗 CLONE DEVICE / KERNEL / VENDOR REPOS
# =========================================================
...
git clone https://github.com/gaurav-paul9/android_device_oneplus_sm8550-common.git -b lineage-23.0 device/oneplus/sm8550-common --depth=1 || true
...
git clone https://github.com/inferno0230/hardware_dolby.git -b sixteen hardware/dolby --depth=1 || true
...
send_raw "✅ *All repos cloned (or already present)* 🧱"

# =========================================================
# 🍳 LUNCH + CLEAN BUILD
# =========================================================
. build/envsetup.sh || { send_raw "❌ envsetup failed"; exit 1; }
send_raw "🔐 *Generating private signing keys...* 🔑"
gk -s || { send_raw "❌ gk -s failed"; exit 1; }
send_raw "✅ *Signing keys generated successfully!*  
_Preparing Axion environment..._ ⚙️"
...
ROM1=$(find out/target/product/aston -type f -name "axion*.zip" | tail -n1 || true)
L1="UPLOAD_ERROR"
S1="0"
if [ -n "$ROM1" ] && [ -f "$ROM1" ]; then
  L1=$(upload_rom "$ROM1")
  S1=$(du -h "$ROM1" | cut -f1)
fi
...
rm -rf kernel/oneplus/sm8550 || true
git clone https://github.com/gaurav-paul9/android_kernel_oneplus_sm8550.git -b newroot kernel/oneplus/sm8550 --depth=1 || true

. build/envsetup.sh || { send_raw "❌ envsetup failed (root)"; exit 1; }
send_raw "🔐 *Generating private signing keys (Rooted Build)...* 🔑"
gk -s || { send_raw "❌ gk -s failed (root)"; exit 1; }
send_raw "✅ *Signing keys ready!*  
_Configuring Axion environment for rooted build..._ ⚙️"
axion aston gms core || true
...
ROM2=$(find out/target/product/aston -type f -name "axion*.zip" | tail -n1 || true)
L2="UPLOAD_ERROR"
S2="0"
if [ -n "$ROM2" ] && [ -f "$ROM2" ]; then
  L2=$(upload_rom "$ROM2")
  S2=$(du -h "$ROM2" | cut -f1)
fi

if [ "$L2" = "UPLOAD_ERROR" ]; then
  send_raw "✅ *Rooted Build Complete!*  
❌ Upload Failed  
